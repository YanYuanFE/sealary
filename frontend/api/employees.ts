...
type Pii = { name: string; title: string; salary: number }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'GET') {
    ...
    const roster = rows.map((r) => {
      ...
      return { id: r.id, walletAddress: r.wallet_address, name: pii.name, title: pii.title, salary: pii.salary }
    })
    return res.json(roster)
  }

  if (req.method === 'POST') {
    const { companyId, name, title, walletAddress, salary, taxId } = req.body ?? {}
    ...
    const cipher = encryptPII({ name, title: title ?? 'Employee', salary } satisfies Pii, dek)
    ...
    return res.json({ id: personId, walletAddress, name, title: title ?? 'Employee', salary })
  }
