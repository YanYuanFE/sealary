...
// GET  → 当前雇主的组织；POST { name, tokenId, symbol, decimals, payDay } → 创建。
export default async function handler(req: VercelRequest, res: VercelResponse) {
  const wallet = await authWallet(req)
  if (!wallet) return res.status(401).json({ error: 'auth' })

  if (req.method === 'GET') {
    const rows = await sql`select id, name, token_id as "tokenId", symbol, decimals, pay_day as "payDay" from company where employer_wallet = ${wallet}`
    return res.json(rows[0] ?? null)
  }

  if (req.method === 'POST') {
    const { name, tokenId, symbol, decimals, payDay } = req.body ?? {}
    ...
    const rows = await sql`
      insert into company (employer_wallet, name, token_id, symbol, decimals, pay_day)
      values (${wallet}, ${name}, ${tokenId}, ${symbol}, ${decimals}, ${pd})
      ...
      returning id, name, token_id as "tokenId", symbol, decimals, pay_day as "payDay"`
